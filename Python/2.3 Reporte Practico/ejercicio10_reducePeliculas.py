...
#hacer un filter para director
director = list(filter(lambda director:director ["Director"]== "Christopher Nolan" ,L_peliculas))

#hacer arreglo con map con titulo y calificacion
pelis = list(map(lambda pelicula: {"titulo": pelicula["Titulo"],"calificacion": pelicula["CalificaciónIMDB"]},director))

#hacer reduce promedio de calificacion
prom = reduce(lambda acumulador, pelicula: acumulador + float(pelicula["calificacion"]), pelis,0.0)

prom2= prom/len(pelis)
